...
public class ReviewController extends HttpServlet {
    ...
    private void checkReviewEligibility(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            int orderID = Integer.parseInt(request.getParameter("orderID"));
            Integer productID = reviewService.getProductIdByOrderId(orderID);
            boolean canUserReviewProduct = false;
            String reviewMessageProduct = "";
            if (productID != null && reviewService.isProductOrderEligibleForReview(productID, user.getUserID())) {
                canUserReviewProduct = true;
            } else {
                reviewMessageProduct = "Bạn chỉ có thể đánh giá sản phẩm sau khi đơn hàng đã được giao và thanh toán thành công.";
            }
            // Set attribute cho JSP nếu cần forward
            request.setAttribute("canUserReviewProduct", canUserReviewProduct);
            request.setAttribute("reviewMessageProduct", reviewMessageProduct);
            // Trả về JSON cho AJAX (nếu dùng)
            StringBuilder json = new StringBuilder();
            json.append("{");
            json.append("\"orderEligible\": ").append(canUserReviewProduct).append(",");
            json.append("\"reviewMessageProduct\": \"").append(reviewMessageProduct).append("\"");
            json.append("}");
