...
public class ProductDAO {
    ...
    public List<entity.Product.ProductImage> getProductImages(int productId) {
        List<entity.Product.ProductImage> images = new ArrayList<>();
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
    ...
    /**
     * Add new product and return the generated product ID
     */
    public int addProduct(Product product) {
        String sql = "{CALL CreateProductFullWithID(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}";
        try (Connection con = DBContext.getConnection(); CallableStatement cs = con.prepareCall(sql)) {
            ...
            // Output parameter for product ID
            cs.registerOutParameter(17, Types.INTEGER);

            cs.execute();

            int productId = cs.getInt(17);
            return productId > 0 ? productId : 0;

        } catch (Exception e) {
            e.printStackTrace();
            // Fallback: use INSERT and return generated key
            return addProductFallback(product);
        }
    }
    
    /**
     * Fallback method to add product using regular INSERT statement
     */
    private int addProductFallback(Product product) {
        String sql = "INSERT INTO Product (name, price, description, stock, status, villageID, categoryID, " +
                    "mainImageUrl, craftTypeID, sku, weight, dimensions, materials, careInstructions, " +
                    "warranty, modelFile, createdDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (Connection con = DBContext.getConnection(); 
            ...
            ps.setString(16, product.getModelFile());
