    ...
    <body>
                            ...
                            <c:forEach var="orderDetail" items="${orderDetails}">
                                <div class="order-card">
                                        ...
                                        <div>
                                            <c:choose>
                                                ...
                                                <c:when test="${orderDetail.orderStatus == 1}">
                                                    <span class="status-badge status-confirmed">Shipping</span>
                                                </c:when>
                                                <c:when test="${orderDetail.orderStatus == 4}">
                                                    <span class="status-badge status-shipping">Refunded</span>
                                                </c:when>
                                                <c:when test="${orderDetail.orderStatus == 5}">
                                                    <span class="status-badge status-shipping">Refunding</span>
                                                </c:when>
                                                <c:when test="${orderDetail.orderStatus == 2}">
                                                    <span class="status-badge status-delivered">Received</span>
                                                </c:when>
                                                <c:when test="${orderDetail.orderStatus == 6}">
                                        ...
                                        <div class="col-md-6">
                                            <div class="customer-info">
                                                ...
                                                <c:forEach var="od" items="${order}">
                                                    <c:if test="${od.id == orderDetail.orderId}">
                                                        <p class="mb-1"><strong>Name:</strong> ${od.shippingName}</p>
                                                        <p class="mb-1"><strong>Phone:</strong> ${od.shippingPhone}</p>
                                                        <p class="mb-0"><strong>Address:</strong> ${od.shippingAddress}</p>
                                                    </c:if>
                                                </c:forEach>
                                                <p class="mb-1"><strong>Order Date:</strong> 
                                                    <fmt:formatDate value="${orderDetail.createdDate}" pattern="dd/MM/yyyy HH:mm"/>
                                                </p>
                                                <p class="mb-0"><strong>Payment Method:</strong> ${orderDetail.paymentMethod}</p>

                                                <c:forEach var="product" items="${productOrder}">
                                                    <c:if test="${product.subOrderId == orderDetail.subOrderId}">
                                                        ...
                                                        <p class="mb-1"><strong>Total:</strong> 
                                                            <fmt:formatNumber value="${product.subtotal}" type="currency" currencySymbol="$"/>
                                                        </p>
                                                    </c:if>
                                                </c:forEach>

                                                <c:forEach var="ticket" items="${ticketOrder}">
                                                    <c:if test="${ticket.subOrderId == orderDetail.subOrderId}">
                                                        <p class="mb-1"><strong>Quantity:</strong> ${ticket.quantity}</p>
                                                        <p class="mb-1"><strong>Product:</strong> ${ticket.villageName}</p>
                                                        <p class="mb-1"><strong>Total:</strong> 
                                                            <fmt:formatNumber value="${ticket.subtotal}" type="currency" currencySymbol="$"/>
                                                        </p>
                                                    </c:if>
                                                </c:forEach>
                                            </div>
                                    ...
                                    <div class="action-buttons">
                                        ...
                                        <c:if test="${not empty orderDetail.refundReason}">
                                            <p class="mb-0"><strong>Reason for Refund/Failure:</strong> 
                                                <span class="text-danger">${orderDetail.refundReason}</span>
                                            </p>
                                        </c:if>

                                            <c:if test="${orderDetail.orderStatus == 0}">
                                                <a href="seller-update-order-status?subOrderId=${orderDetail.subOrderId}&action=confirmOrder">Confirm</a> 
                                                <a href="seller-update-order-status?subOrderId=${orderDetail.subOrderId}&action=cancelOrder">Reject</a> 
                                            </c:if>

                                            <c:if test="${orderDetail.orderStatus == 4}">
                                                <a href="seller-update-order-status?subOrderId=${orderDetail.subOrderId}&action=refundOrder">Confirm</a> 
                                                <a href="seller-update-order-status?subOrderId=${orderDetail.subOrderId}&action=rejectOrder">Reject</a> 
                                            </c:if>
                                    </div>
                                </div>
                            </c:forEach>
        ...
        <script>
                                                            function updateOrderStatus(orderDetailId, action) {
                                                                Swal.fire({
                                                                    title: 'Are you sure?',
                                                                    text: `Do you want to ${action.replace('Order', '')} this order?`,
                                                                    icon: 'warning',
