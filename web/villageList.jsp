...
<head>
    ...
    <title>Village Management - ${sessionScope.acc.userName}</title>
...
<body>
    ...
    <main class="main-content">
        <header class="main-header">
            ...
            <p>Manage, add, edit, or hide village information in the system.</p>
        </header>

        <section class="panel">
            <div class="panel-header">
                <h2 class="panel-title">Village List</h2>
                <a href="<c:url value='/manage-villages?action=add'/>" class="primary-action-btn">
                    <i class="fas fa-plus"></i> Add New Village
                </a>
            </div>
            ...
            <div class="search-filter-box">
                <form action="<c:url value='/manage-villages'/>" method="get" class="filter-form">
                    <input type="hidden" name="action" value="search">
                    <div class="filter-group">
                        <label for="id">Village ID</label>
                        <input type="text" id="id" name="id" value="${SEARCH_VALUE}" placeholder="Enter ID...">
                    </div>
                    <div class="filter-group">
                        <label>&nbsp;</label>
                        <div style="display: flex; gap: 0.75rem;">
                            <button type="submit" class="primary-action-btn"><i class="fas fa-filter"></i> Filter</button>
                            <a href="<c:url value='/manage-villages'/>" class="secondary-action-btn">Clear Filter</a>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <table class="product-table">
                ...
                <tbody>
                    <c:if test="${empty VILLAGE_LIST}">
                        <tr><td colspan="6" style="text-align: center; padding: 3rem;">No villages found.</td></tr>
                    </c:if>
                    <c:forEach var="v" items="${VILLAGE_LIST}">
                        <tr>
                            <td>${v.villageID}</td>
                            <td>
                                <img src="${v.mainImageUrl}" alt="${v.villageName}" onerror="this.src='https://placehold.co/70x70/E0D9CF/3D3D3D?text=Image'">
                            </td>
                            <td><c:out value="${v.villageName}"/></td>
                            ...
                            <td class="action-buttons">
                                <a href="<c:url value='/manage-villages?action=edit&id=${v.villageID}'/>" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="<c:url value='/manage-villages'/>" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to hide this village?');">
                                    <input type="hidden" name="action" value="delete">
                                    <input type="hidden" name="id" value="${v.villageID}">
                                    <button type="submit" title="Hide village">
