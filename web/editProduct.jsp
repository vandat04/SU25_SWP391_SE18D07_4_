...
<head>
    ...
    <title>Chỉnh Sửa Tác Phẩm - <c:out value="${product.name}"/></title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-actions { margin-top: 2.5rem; display: flex; justify-content: flex-end; gap: 1rem; grid-column: 1 / -1; }
        .action-button { background-color: var(--accent-color); color: #fff; border: none; padding: 0.8rem 2rem; border-radius: 8px; font-family: var(--font-body); font-weight: 700; font-size: 1rem; text-decoration: none; cursor: pointer; transition: background-color 0.3s, transform 0.2s; }
        .action-button:hover { background-color: var(--accent-hover); transform: translateY(-2px); }
        .action-button.secondary { background-color: #f0f0f0; color: var(--primary-text); }
        .action-button.secondary:hover { background-color: #e0e0e0; }
        .alert-error { padding: 1rem 1.5rem; margin-bottom: 2rem; border-radius: 8px; border: 1px solid var(--error-border); background-color: var(--error-bg); color: var(--error-text); font-weight: 500; }
    </style>
</head>
<body>
    ...
    <aside class="sidebar">
        ...
        <div class="sidebar-profile">
            ...
            <h3><c:out value="${sessionScope.acc.userName}"/></h3>
            <p>Nghệ nhân</p>
        </div>
        ...
        <div class="sidebar-logout"><a href="<c:url value='/logout'/>" style="padding: 1rem 2rem;"><i class="fas fa-sign-out-alt" style="margin-right: 1rem; width: 25px; text-align: center;"></i> Đăng xuất</a></div>
    </aside>

    <main class="main-content">
        <header class="main-header">
            <h1>Chỉnh Sửa Tác Phẩm</h1>
            <p>Cập nhật thông tin chi tiết cho tác phẩm: <strong><c:out value="${product.name}"/></strong></p>
        </header>
            ...
            <form action="<c:url value='/editProduct'/>" method="post">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group full-width">
                        ...
                        <input id="name" type="text" name="name" class="form-control" required value="<c:out value='${product.name}'/>">
                    </div>

                    <div class="form-group">
                        <label for="sku">Mã sản phẩm (SKU)</label>
                        <input id="sku" type="text" name="sku" class="form-control" value="<c:out value='${product.sku}'/>">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="categoryId" name="categoryId" class="form-control">
                            <c:forEach var="cat" items="${categoryList}">
                                <option value="${cat.categoryID}" ${product.categoryID == cat.categoryID ? 'selected' : ''}><c:out value="${cat.categoryName}"/></option>
                            </c:forEach>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input id="price" type="number" step="1000" min="0" name="price" class="form-control" required value="${product.price}">
                    </div>

                    <div class="form-group">
                        <label for="stock">Số lượng tồn kho</label>
                        <input id="stock" type="number" min="0" name="stock" class="form-control" required value="${product.stock}">
                    </div>
                    
                    <div class="form-group">
                        <label for="weight">Trọng lượng (gram)</label>
                        <input id="weight" type="number" step="1" min="0" name="weight" class="form-control" value="${product.weight}">
                    </div>

                    <div class="form-group">
                        <label for="dimensions">Kích thước (D x R x C)</label>
                        <input id="dimensions" type="text" name="dimensions" class="form-control" value="<c:out value='${product.dimensions}'/>">
                    </div>
                    
                    <div class="form-group full-width">
                        <label for="description">Mô tả chi tiết</label>
                        <textarea id="description" name="description" class="form-control" rows="6"><c:out value='${product.description}'/></textarea>
                    </div>

                    <div class="form-group full-width">
                        <label for="materials">Chất liệu</label>
                        <input id="materials" type="text" name="materials" class="form-control" value="<c:out value='${product.materials}'/>">
                    </div>

                    <div class="form-group full-width">
                        <label for="careInstructions">Hướng dẫn bảo quản</label>
                        <textarea id="careInstructions" name="careInstructions" class="form-control" rows="4"><c:out value='${product.careInstructions}'/></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label for="warranty">Thông tin bảo hành</label>
                        <input id="warranty" type="text" name="warranty" class="form-control" value="<c:out value='${product.warranty}'/>">
                    </div>

                    <div class="form-group">
                        <label for="mainImageUrl">URL Hình ảnh chính</label>
                        <input id="mainImageUrl" type="text" name="mainImageUrl" class="form-control" value="<c:out value='${product.mainImageUrl}'/>">
                    </div>
                    
                    <div class="form-group">
                        <div class="form-check" style="padding-top: 2.25rem;">
                    ...
                    <div class="form-actions">
                         <a href="<c:url value='/manageProduct'/>" class="action-button secondary">Hủy</a>
